  ...
  return (
    <div className="flex min-h-dvh items-center justify-center bg-gradient-to-b from-gray-100 to-white p-6">
      <form onSubmit={submit} className="w-full max-w-sm rounded-2xl border border-gray-200 bg-white p-6 shadow-lg">
        ...
        {setupRequired && <><label className="mb-1 block text-xs font-medium text-gray-600" htmlFor="staff-display-name">ชื่อที่แสดง</label><input id="staff-display-name" required minLength={2} value={displayName} onChange={(event) => setDisplayName(event.target.value)} className="mb-4 w-full rounded-xl border border-gray-200 px-3 py-3 text-sm outline-none focus:border-brand" /></>}
        ...
        <div className="relative mb-1"><LockKeyhole className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" /><input id="staff-password" type="password" autoComplete={setupRequired ? "new-password" : "current-password"} required minLength={setupRequired ? 10 : undefined} value={password} onChange={(event) => setPassword(event.target.value)} className="w-full rounded-xl border border-gray-200 py-3 pl-10 pr-3 text-sm outline-none focus:border-brand" /></div>
        {setupRequired && <p className="mb-4 text-[11px] text-gray-500">ใช้อย่างน้อย 10 ตัวอักษร และไม่ควรใช้รหัสเดียวกับบัญชีส่วนตัว</p>}
        {error && <p className="mb-4 rounded-lg bg-red-50 px-3 py-2 text-xs text-red-600">{error}</p>}
        <button disabled={submitting || loading} className="flex w-full items-center justify-center gap-2 rounded-xl bg-gray-800 py-3 text-sm font-medium text-white disabled:opacity-50">{submitting ? <Loader2 className="h-4 w-4 animate-spin" /> : <LogIn className="h-4 w-4" />} {submitting ? "กำลังบันทึก..." : setupRequired ? "สร้างบัญชีผู้จัดการ" : "เข้าสู่ระบบ"}</button>
      </form>
    </div>
  );
}
